...
module.exports = NodeHelper.create({
  ...
  activateMonitor: function () {
    ...
    let alwaysOffTrigger =
      this.alwaysOff &&
      this.alwaysOff.readSync() === this.config.alwaysOffState;
    if (alwaysOffTrigger) {
      return;
    }
    ...
    if (this.config.relayPin !== false) {
      ...
      exec("/usr/bin/vcgencmd display_power").stdout.on(
        ...
        function (data) {
          ...
          if (self.config.supportCEC) exec("echo 'on 0' | cec-client -s -d 1");
        }
      );
    }
  ...
  deactivateMonitor: function () {
    ...
    let alwaysOnTrigger =
      this.alwaysOn && this.alwaysOn.readSync() === this.config.alwaysOnState;
    let alwaysOffTrigger =
      this.alwaysOff &&
      this.alwaysOff.readSync() === this.config.alwaysOffState;
    ...
    if (this.config.relayPin !== false) {
      ...
      if (this.config.supportCEC) exec("echo 'standby 0' | cec-client -s -d 1");
      exec("/usr/bin/vcgencmd display_power 0", null);
    }
    if (
      this.config.preventHDMITimeout > 0 &&
      this.config.preventHDMITimeout < 10
    ) {
      const self = this;
  ...
  briefHDMIWakeup: function () {
    ...
    exec("/usr/bin/vcgencmd display_power").stdout.on("data", function (data) {
      if (data.indexOf("display_power=0") === 0) {
        ...
        self.briefHDMIWakeupPhase2Timeout = setTimeout(function () {
          // set a short delay for the monitor to sense the input
    ...
    if (notification === "CONFIG" && this.started == false) {
      ...
      if (self.config.powerSaving) {
        self.deactivateMonitorTimeout = setTimeout(function () {
          // Set the timeout before movement is identified
      ...
      if (this.config.relayPin) {
        ...
        this.relay.writeSync(this.config.relayState);
        exec("/usr/bin/vcgencmd display_power 1", null);
        if (this.config.supportCEC) exec("echo 'on o' | cec-client -s -d 1");
      }
